//Thanks to P_Malin. Link: https://www.shadertoy.com/view/lt3GRj

float DigitBin(const int x)
{
    return x==0?480599.0:x==1?139810.0:x==2?476951.0:x==3?476999.0:x==4?350020.0:x==5?464711.0:x==6?464727.0:x==7?476228.0:x==8?481111.0:x==9?481095.0:0.0;
}

float PrintValue2(float2 fragCoord, float2 pixelCoord, float2 fontSize, float fValue, float digits, float decimals)
{
    float2 charCoord = (fragCoord - pixelCoord) / fontSize;
    if(charCoord.y < 0.0 || charCoord.y >= 1.0) return 0.0;
    float bits = 0.0;
    float digitIndex1 = digits - floor(charCoord.x)+ 1.0;
    if(- digitIndex1 <= decimals) {
        float pow1 = pow(10.0, digitIndex1);
        float absValue = abs(fValue);
        float pivot = max(absValue, 1.5) * 10.0;
        if(pivot < pow1) {
            if(fValue < 0.0 && pivot >= pow1 * 0.1) bits = 1792.0;
        } else if(digitIndex1 == 0.0) {
            if(decimals > 0.0) bits = 2.0;
        } else {
            fValue = digitIndex1 < 0.0 ? frac(absValue) : absValue * 10.0;
            bits = DigitBin(int (fmod(fValue / pow1, 10.0)));
        }
    }
    return floor(fmod(bits / pow(2.0, floor(frac(charCoord.x) * 4.0) + floor(charCoord.y * 5.0) * 4.0), 2.0));
}

void Debug_float(float4 val, float2 uv, float2 textPos, float textScale, out float4 res)
{
    //res = float4(0.3,0.2,0.1,1);

    float a = PrintValue2(uv*textScale, float2(0, 60) + textPos, float2(8,15), val.x, 10, 3);
    float b = PrintValue2(uv*textScale, float2(0, 40) + textPos, float2(8,15), val.y, 10, 3);
    float c = PrintValue2(uv*textScale, float2(0, 20) + textPos, float2(8,15), val.z, 10, 3);
    float4 vec = float4(a, b, c, 1);
    float4 d = PrintValue2(uv*textScale, float2(0, 0) + textPos, float2(8,15), val.w, 10, 3);

    res = vec + d;
}