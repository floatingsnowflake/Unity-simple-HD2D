...
namespace TeamMingo.ImagePacker.Editor
{
    ...
    internal class ImagePackNode
    {
        ...
        public bool TryInsert(ImagePackUtils.ImagePackRect insert, int padding, out Vector2Int remainingSpace)
        {
        ...
        static bool SplitRects(ImagePackNode node, ImagePackUtils.ImagePackRect insert, int padding, out ImagePackNode left, out ImagePackNode right)
        {
        ...
        public bool Insert(ImagePackUtils.ImagePackRect insert, int padding)
        {
            int insertWidth = insert.rect.width + padding * 2;
            int insertHeight = insert.rect.height + padding * 2;
            if (insertWidth > rect.width || insertHeight > rect.height)
