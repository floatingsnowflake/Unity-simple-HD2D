...
using System;
using System.Linq;
using TeamMingo.ImagePacker.Editor.Jobs;
using Unity.Collections;
using Unity.Collections.LowLevel.Unsafe;
using UnityEngine;

namespace TeamMingo.ImagePacker.Editor
{
    public static class ImagePackUtils
    {
        /// <summary>
        ...
        public static void Pack(NativeArray<Color32>[] buffers, int width, int height, int padding, bool tight, out NativeArray<Color32> outPackedBuffer, out int outPackedBufferWidth, out int outPackedBufferHeight, out RectInt[] outPackedRect, out Vector2Int[] outUVTransform)
        {
            ...
            try
            {
                ...
                if (tight)
                {
                    tightRects = FindTightRectJob.Execute(buffers, width, height);
                }
                else
                {
                    tightRects = buffers.Select(_ => new RectInt(0, 0, width, height)).ToArray();
                }
